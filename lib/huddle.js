...
function Huddle(options) {
     ...
     * @method writeScripts
     */
    Huddle.prototype.writeScripts = function (output) {
        var i,
            j,
            k,
            l,
        ...
        for (i in self._scripts) {
            ...
            data.mangle_names();
            fs.writeFileSync(path.dirname(output) + '/' + i + '.js',
                             data.print_to_string(), 'utf8');
        }
    }
     ...
     */
    Huddle.prototype.writeStylesheets = function (output) {
        var i,
            j,
            data;
        for (i in self._stylesheets) {
            ...
            function checkRender() {
                if (toWait === 0) {
                    fs.writeFileSync(path.dirname(output) + '/' + i + '.css',
